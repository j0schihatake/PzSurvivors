PzSurvivorTaskManager = {}
PzSurvivorTaskManager.__index = PzSurvivorTaskManager

function PzSurvivorTaskManager:new(pzSurvivor)
...
--Установить лимит тасок:
function PzSurvivorTaskManager:setTaskUpdateLimit(toValue)
    self.TaskUpdateLimit = toValue
    self.TaskUpdateCount = 0
end

function PzSurvivorTaskManager:AddToTop(newTask)
...
function PzSurvivorTaskManager:AddToBottom(newTask)
...
function PzSurvivorTaskManager:Display()
    for i=1,self.TaskCount-1 do
        if (self.Tasks[i] ~= nil) then return print(self.Tasks[i].Name) end
    end
end

function PzSurvivorTaskManager:clear()
    self.TaskCount = 0
    self.Tasks[0] = nil
end

function PzSurvivorTaskManager:moveDown()
...
function PzSurvivorTaskManager:getCurrentTask()
    if (self.Tasks[0] ~= nil) and (self.Tasks[0].Name ~= nil) then return self.Tasks[0].Name
    else return "None" end
end

function PzSurvivorTaskManager:getTask()
    if (self.Tasks[0] ~= nil) then return self.Tasks[0]
    else return nil end
end

function PzSurvivorTaskManager:getThisTask(index)
    if (self.Tasks[index] ~= nil) then return self.Tasks[index]
    else return nil end
end

function PzSurvivorTaskManager:getTaskFromName(thisName)
...
function PzSurvivorTaskManager:update()
